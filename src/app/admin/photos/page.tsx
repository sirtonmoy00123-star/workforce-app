    ...
    <div className="max-w-4xl mx-auto">
      ...
      {data && (
        <div className="bg-white rounded-2xl border border-gray-200 p-5 mb-4">
          ...
          <div className="flex items-center gap-3 mb-4">
            ...
            <div>
              ...
              <div className="text-sm text-gray-500">
                photos{filterEmployee ? " (filtered)" : " across all employees"} · ~{data.estimatedStorageMB} MB
              </div>
            </div>
          </div>
          ...
          {/* By employee */}
          {!filterEmployee && data.byEmployee.length > 0 && (
            <>
              ...
              <div className="space-y-1.5">
                {data.byEmployee.map((emp) => (
                  <button
                    key={emp.employeeId}
                    onClick={() => {
                      setFilterEmployee(emp.employeeId);
                      setSelected(new Set());
                    }}
                    className="flex items-center justify-between w-full py-2 px-2 hover:bg-gray-50 rounded-lg transition-colors text-left"
                  >
      ...
      {data && data.totalPhotos > 0 && (
        <div className="bg-white rounded-2xl border border-gray-200 p-5 mb-4">
          ...
          <p className="text-sm text-gray-500 mb-4">
            Delete photos from completed shifts older than a set number of days.
            {filterEmployee
              ? " Deleting for the selected employee only."
              : " Applies to all employees."}
            {" "}Only completed shift photos are deleted — active/pending shifts are safe.
          </p>
          ...
          >
            ...
            {filterEmployee && data.employees.length > 0
              ? ` (${data.employees.find((e) => e.id === filterEmployee)?.name || "Selected Employee"})`
              : " (All Employees)"}
          </button>
        </div>
      )}
        ...
        <div className="bg-white rounded-2xl border border-gray-200 p-5 mb-4">
          <div className="flex items-center justify-between mb-3">
            <h2 className="font-semibold text-gray-900">
              Recent Photos {filterEmployee ? "(Filtered)" : ""}
            </h2>
            <div className="flex items-center gap-2">
                ...
                >
                  ...
                  <div className="absolute bottom-0 left-0 right-0 bg-black/60 px-1.5 py-1">
                    <div className="text-[10px] text-white font-medium truncate">
                      {typeInfo.emoji} {photo.employeeName.split(" ")[0]}
                    </div>
      ...
      {data && data.totalPhotos === 0 && (
        <div className="text-center py-12 bg-white rounded-2xl border border-gray-200">
          ...
          <p className="text-gray-500 text-sm">
            {filterEmployee ? "No photos found for this employee." : "No photos stored yet."}
          </p>
        </div>
      )}
          ...
          <div className="bg-white rounded-2xl w-full max-w-md p-5">
            ...
            <p className="text-sm text-gray-500 mb-4">
              {showConfirm === "selected"
                ...
                : `Delete all photos from completed shifts older than ${bulkDays} days${
                    filterEmployee
                      ? ` for ${data?.employees.find((e) => e.id === filterEmployee)?.name || "this employee"}`
                      : " for all employees"
